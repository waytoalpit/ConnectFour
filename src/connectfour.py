...
class ConnectFourBoard(object):
    ...
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1, kVal=4, longStreak=False, streakLen=20):
        ...
        """
        self._kVal=kVal
        self._longStreak=longStreak
        self._streakLen = streakLen
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(), kVal=self._kVal, longStreak=self._longStreak, streakLen=self._streakLen)
    ...
    def _is_win_from_cell(self, row, col):
        ...
        return ( self._max_length_from_cell(row, col) >= self._kVal )
    
    def _is_win_from_cell_longStreak(self, row, col):
        """ Determines if there is a winning set of four connected nodes containing the specified cell """
    ...
    def is_win(self):
        ...
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                ...
                if cell_player != 0:
                    global number_Of_Expansion
                    if self._longStreak and number_Of_Expansion >= self._streakLen:
                        newLen = self._is_win_from_cell_longStreak(i,j)
                        if newLen > curLen:
                            curLen = newLen
                            winPlayer = cell_player
                                                    
                    if self._longStreak is False:
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(), kVal=self._kVal, longStreak=self._longStreak, streakLen=self._streakLen)
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def run_game(self, verbose=True):
        ...
        if win_for_player != 0 and self._board.is_tie():
            ...
            tEnd = time.clock()
            totTime = tEnd - tStart
            print ("Execution Time: " + str(totTime))
            print ("Total number of nodes Expanded: " + str(total_Nodes_Expanded))
            return 0
        else:
